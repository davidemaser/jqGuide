...
(function ( $ ) {
    $.fn.jqguide = function( options ) {
        ...
        var  styleBlock = '<style type="text/css">#left-panel:before,.item_title{text-align:center;font-size:40px}section#global{position:absolute;width:100%;height:100%}#left-panel,#main-panel{display:block;height:100%;position:relative;float:left;overflow-y:auto}#left-panel{width:20%;background:#afafbd}#left-panel:before{content:"Functions";display:block;padding:10px 5px}progress{position:fixed;left:0;bottom:0;width:100%;height:5px;z-index:99;-webkit-appearance:none;-moz-appearance:none;appearance:none;border:none;background-color:transparent;color:#cb0032}progress::-webkit-progress-bar{background-color:transparent}progress::-webkit-progress-value{background-color:#cb0032}progress::-moz-progress-bar{background-color:#cb0032}';
        ...
        if(settings.showProgress == true){
            var htmlObj = '<progress value="0"></progress><section id="global">';
        }else{
            var htmlObj = '<section id="global">';
        }
        if(settings.createNav == true) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for(j=0;j<data['element'].length;j++){
                    var view = '<div class="item_layout">';
                    ...
                    if(typeof(data["element"][j]["parameters"]) == 'object' && data["element"][j]["parameters"] !== null){
                        view += '<div class="item_subline get_params">';
                        for(k=0;k<data["element"][j]["parameters"].length;k++){
                    ...
                    if(typeof(data["element"][j]["dependencies"]) == 'object' && data["element"][j]["dependencies"] !== null){
                        view += '<div class="item_subline get_depends">';
                        for(l=0;l<data["element"][j]["dependencies"].length;l++){
            ...
            complete:function (){
                sortNav();
                var winHeight = $('#main-panel').height(),
